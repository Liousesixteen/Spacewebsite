...
export default async function AdminSpacecraftPage({ params, searchParams }: Props) {
  const { locale } = await params;
  const sp = await searchParams;
  const q = (sp.q ?? '').trim();
  const page = Math.max(1, parseInt(sp.page ?? '1', 10) || 1);

  const where: Prisma.SpacecraftWhereInput = q
    ? {
        OR: [
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-white">航天器</h1>
          <p className="text-sm text-star-dim mt-1">共 {total} 条记录</p>
        </div>
        <div className="flex items-center gap-3">
          <SearchBox placeholder="按名称/运营方..." />
          <Link href={`/${locale}/admin/spacecraft/new`}>
            <Button>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left border-b border-space-700 text-star-dim text-xs uppercase tracking-wider">
                  ...
                  <th className="px-5 py-3">运营方</th>
                  <th className="px-5 py-3">发射日期</th>
              ...
              <tbody>
                ...
                {items.map((s) => (
                  <tr key={s.id} className="border-b border-space-700/60 hover:bg-space-800/40">
                    <td className="px-5 py-3 text-white">{s.name}</td>
                    <td className="px-5 py-3 text-star-dim">{s.type}</td>
                    <td className="px-5 py-3 text-star-dim">{s.operator}</td>
                    <td className="px-5 py-3 text-star-dim">
                      {format(new Date(s.launchDate), 'yyyy-MM-dd')}
                    </td>
